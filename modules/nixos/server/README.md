...
| `reverse-proxy.nix`| **Nginx Proxy Manager** container (LE certs + proxy hosts via its web UI). Public HTTP/HTTPS on `80`/`443`; admin UI published on the VPN address only (`10.100.0.1:81`). Data under `/hdd_pool_1/services/npm`. Certs via HTTP-01. Hardened: no socket mount, `no-new-privileges`. Its nginx access/error logs are files under `data/logs` — the only logs on this box outside journald — so `npm-data-dirs` gives that directory to the `npm-logs` group (setgid, so new proxy hosts' files inherit it) for `logs.nix` to tail, and a `logrotate` rule with `copytruncate` keeps them bounded. |
...
| `metrics.nix`      | **Prometheus** (`:9090`) plus four exporters: **node** (`:9100`), **smartctl** (`:9633`), **postgres** (`:9187`) and **blackbox** (`:9115`) — CPU/memory/disk/network/ZFS-ARC metrics, systemd unit and timer state, SMART drive health, the shared cluster, and end-to-end HTTPS probes of the public hostnames (uptime + TLS expiry). All bind `127.0.0.1` and are never firewalled open. The postgres exporter carries **no credential**: `runAsLocalSuperUser` runs it as the `postgres` system user over the Unix socket, where peer auth does the rest. Blackbox is the one job that is not a plain `host:port` scrape — the probe URL goes in as `?target=` and `relabel_configs` rewrites `__address__` back to the exporter. TSDB on the SSD (`/var/lib/prometheus2`, `services.prometheus.stateDir` is `/var/lib`-relative), bounded at 90 days / 20 GiB. |
| `logs.nix`         | **Loki** log store (`:3100`) + **Grafana Alloy** shipping logs into it. Two sources: the **systemd journal**, which already covers every service on the box (they are all native units) plus sudo and — via `auditd.plugins.syslog` — the audit trail; and **NPM's nginx access/error log files** on the pool (`job="npm"`, labels `vhost`/`status`/`method`; the client IP stays in the line, never a label). Alloy runs under `DynamicUser`, so it reads those files through the `npm-logs` group declared here. `:3100` is push *and* query with no authentication, so it is admitted by a source-restricted rule from the podman bridge, LAN and VPN only — that restriction *is* the access control. Chunks, index and compactor on the pool under `/hdd_pool_1/services/loki`, 90-day retention. |
| `grafana.nix`      | **Grafana** (`:3030` — off the reserved 3000-3010 range), admitted **only on `wg0`** and deliberately *not* proxied through NPM, so it has no WAN surface (`http://10.100.0.1:3030`). Prometheus + Loki datasources **and** four dashboards (`./dashboards/*.json`: host, services, logs, HTTP) provisioned from Nix and therefore read-only in the UI — edit the JSON and rebuild. Dashboards built by hand in the browser live in the database and are untouched. SQLite state on the pool at `/hdd_pool_1/services/grafana`. Two sops secrets in `secrets/home-server/grafana.yaml` (**bare values**, owned by `grafana`, read via Grafana's `$__file{}` provider): `grafana-admin-password`, and `grafana-secret-key` — the key encrypting secrets *inside* Grafana's database, which must not be rotated once anything is stored (and `alerts.nix` now stores something). |
| `alerts.nix`       | **Alerting**: Grafana unified alerting, rules + contact point + notification policy + payload template all provisioned from Nix — no Alertmanager, no SMTP. Nine rules (disk and pool free space, failed units, dead scrape targets, stale nightly backups, SMART failure, public-endpoint downtime, TLS expiry, and one Loki rule on SSH auth failures) route to the **ntfy** server this box already runs, over loopback to `:2586`. The webhook uses a custom payload template emitting ntfy's JSON publishing format, so notifications arrive readable rather than as a dump of Grafana's envelope. One sops secret in `secrets/home-server/ntfy.yaml`: `ntfy-alert-token`, in **`NTFY_ALERT_TOKEN=…` env-file format** (not a bare value — the opposite of the Grafana secrets), read via `grafana.service`'s `EnvironmentFile` and interpolated into the provisioning YAML. Ships a placeholder; issue the real token with `ntfy token add svc` (INSTALL.md §8). |
