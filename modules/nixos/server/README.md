...
| `metrics.nix`      | **Prometheus** (`:9090`) plus the **node** (`:9100`) and **smartctl** (`:9633`) exporters — CPU/memory/disk/network/ZFS-ARC metrics, systemd unit state, and SMART drive health for the pool disks. All three bind `127.0.0.1` and are never firewalled open: Grafana is the only reader and runs on the same host. TSDB on the SSD (`/var/lib/prometheus2`, `services.prometheus.stateDir` is `/var/lib`-relative), bounded at 90 days / 20 GiB. |
...
| `grafana.nix`      | **Grafana** (`:3030` — off the reserved 3000-3010 range), admitted **only on `wg0`** and deliberately *not* proxied through NPM, so it has no WAN surface (`http://10.100.0.1:3030`). Prometheus + Loki datasources provisioned from Nix; dashboards are imported in the UI. SQLite state on the pool at `/hdd_pool_1/services/grafana`. Admin password is a sops secret (`secrets/home-server/grafana.yaml`, key `grafana-admin-password`, **bare password**, owned by `grafana`). |
...
`paperless.nix`, `logs.nix` and `grafana.nix`. Their five directory-creating
oneshots have no ordering between them, so they all leave that parent as
`root:root 0755` — traversable, so each unprivileged service reaches its own
subtree, which stays `0750`. Changing that mode in one module without the others
