...
| File                     | Configures                                                                                                                                                                 |
|--------------------------|--------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| `home-server-client.nix` | `services.homeServerClient`: WireGuard **client** of the home-server (`wg0`, split-tunnel to `10.100.0.0/24`), `known_hosts` pin for `home-server`, and an `x-systemd.automount` NFS mount of the share at `/mnt/home-server`. Also a **`wg0-reresolve` timer** (1 min) for hosts whose `endpoint` is a *hostname*: WireGuard resolves `Endpoint` once at interface-up and then keeps sending to that address, so every ISP reconnect at the far end leaves the tunnel silently pointed at a stale WAN address. The timer re-resolves and re-sets the endpoint **only when the peer's last handshake is older than 150 s** — re-setting a healthy peer would clobber the address WireGuard learned from the peer's own roaming. A host pointed at an address literal (the desktop, on the server's LAN) gets no unit at all; `flake.nix` asserts that equivalence in both directions. Gated by `enable` — off until the host is enrolled (per-host private key via sops), so importing it keeps keyless CI green. |
